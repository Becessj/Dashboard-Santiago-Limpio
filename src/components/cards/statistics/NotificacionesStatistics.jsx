...
export default function NotificacionesStatistics({ color = 'primary', title, count, percentage, isLoss, extra, imageUrl }) {
  ...
  useEffect(() => {
    const fetchNotificationCount = async () => {
        ...
        try {
            const response = await fetch("http://localhost:3001/api/countnotifications");
            if (!response.ok) {
...
NotificacionesStatistics.propTypes = {
